...
const byDateAsc = (a, b) => Date.parse(a) - Date.parse(b);
const byDateDesc = (a, b) => Date.parse(b) - Date.parse(a);
const riskToColor = (riskLevel) => {
    if (riskLevel === "high") {
        return "rose";
    }
    if (riskLevel === "medium") {
...
const getRiskBand = (highRiskCount) => {
    if (highRiskCount >= 3) {
        return "critical";
    }
    if (highRiskCount >= 1) {
...
const userIdFromEmail = (email) => {
    const digest = (0, crypto_1.createHash)("sha256").update(email).digest("hex").slice(0, 12);
    return `user_${digest}`;
};
...
const getStateOrThrow = (userId) => {
    const state = userStates.get(userId);
    if (!state) {
        throw new http_1.AppError(401, "SESSION_STATE_MISSING", "Demo workspace is missing. Please login again.");
    }
    return state;
};
const findSubscriptionOrThrow = (state, subscriptionId) => {
    const target = state.subscriptions.find((item) => item.id === subscriptionId);
    if (!target) {
        throw new http_1.AppError(404, "SUBSCRIPTION_NOT_FOUND", "Subscription does not exist.");
    }
    return target;
};
const getOrCreateDemoUser = (email) => {
...
exports.getOrCreateDemoUser = getOrCreateDemoUser;
const ensureDemoStateForUser = (user) => {
    if (userStates.has(user.id)) {
        return;
    }
    userStates.set(user.id, createInitialState(user));
};
exports.ensureDemoStateForUser = ensureDemoStateForUser;
const resetAllDemoStates = () => {
    userStates.clear();
};
exports.resetAllDemoStates = resetAllDemoStates;
const resetDemoStateForUser = (userId) => {
    const currentState = getStateOrThrow(userId);
    userStates.set(userId, createInitialState(currentState.user));
};
exports.resetDemoStateForUser = resetDemoStateForUser;
const getDemoUser = (userId) => structuredClone(getStateOrThrow(userId).user);
...
const listSubscriptions = (userId, options) => {
    const state = getStateOrThrow(userId);
...
const getSubscriptionDetail = (userId, subscriptionId) => {
    const state = getStateOrThrow(userId);
...
exports.getSubscriptionDetail = getSubscriptionDetail;
const startCancellation = (userId, subscriptionId) => {
    const state = getStateOrThrow(userId);
    const subscription = findSubscriptionOrThrow(state, subscriptionId);
    if (subscription.status === "cancelled") {
        return (0, exports.getSubscriptionDetail)(userId, subscriptionId);
    }
...
exports.startCancellation = startCancellation;
const completeCancellation = (userId, subscriptionId) => {
    const state = getStateOrThrow(userId);
    const subscription = findSubscriptionOrThrow(state, subscriptionId);
    let cancellation = state.cancellations.find((entry) => entry.subscriptionId === subscriptionId);
    if (!cancellation) {
...
exports.completeCancellation = completeCancellation;
const setAutoBlock = (userId, subscriptionId, enabled) => {
    const state = getStateOrThrow(userId);
...
exports.setAutoBlock = setAutoBlock;
const getRenewalCalendar = (userId) => {
    const state = getStateOrThrow(userId);
