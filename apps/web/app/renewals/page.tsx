      ...
      <section className="grid gap-4 md:grid-cols-[1fr_1.2fr]">
        <article className="glass-card reveal p-6" style={{ animationDelay: "80ms" }}>
          ...
          {loading ? (
            ...
            <>
              <p className="mt-4 text-4xl font-bold text-slate-100">{formatCurrency(totalUpcomingExposure)}</p>
        ...
        <article className="glass-card reveal p-6" style={{ animationDelay: "120ms" }}>
          <h3 className="text-lg font-semibold text-slate-100">Charge Timeline</h3>
              ...
              >
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="text-base font-semibold text-slate-100">{event.merchant}</p>
                ...
                <p className="text-lg font-bold text-slate-100">{formatCurrency(event.amount)}</p>
              </div>
            ))}

            {!loading && events.length === 0 ? (
              <p className="rounded-lg bg-slate-900/70 px-3 py-2 text-sm text-slate-300">No upcoming renewals found.</p>
            ) : null}
          </div>
        </article>
      </section>
